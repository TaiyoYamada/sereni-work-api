...
- 最適化処理（optimizer/）: Python + OpenJij / dimod / OR-Tools / D-Wave Ocean SDK。別の AWS Lambda（コンテナ、Amazon ECR）としてデプロイし、Hono 側から呼び出す
...
modules/participants/
  ...
  participants.schema.ts      # Zod スキーマ。drizzle-zod から導出して .openapi() を付与
...
- optimizer: QUBO 生成テスト / 制約違反テスト / 小規模問題での全探索との比較 / OR-Tools との比較 / 再現性テスト（seed 固定）/ ソルバー差し替えテスト / 解のデコードテスト / スコア再計算テスト / 実機接続はモックする
